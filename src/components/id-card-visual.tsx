...
export function IdCardVisual({ 
  ...
  return (
    <div style={cardStyle} className="rounded-2xl shadow-2xl border select-none">
      ...
      {showQr && (
        <div className="absolute bg-white p-1.5 rounded-xl border shadow-sm z-10" style={{ left: els.qr.x, top: els.qr.y, width: els.qr.w, height: els.qr.h }}>
      ...
      {(showInfo || showValid) && (
        <div className="absolute px-4 flex flex-col gap-1 z-10" style={{ left: els.info.x, top: els.info.y, width: `${els.info.width || 236}px`, textAlign: els.info.align || 'center', alignItems: els.info.align === 'center' ? 'center' : (els.info.align === 'right' ? 'flex-end' : 'flex-start') }}>
          {showInfo && (
            <>
              <h1 className="font-black uppercase tracking-tight leading-none mb-1" style={{ fontSize: (els.info.fontSize || 12) + 2 }}>{student.name}</h1>
              <div className="inline-block px-3 py-1 rounded-full font-bold text-white uppercase tracking-wider shadow-sm mb-1" style={{ backgroundColor: current.headerBg, fontSize: (els.info.fontSize || 12) - 3 }}>{student.major}</div>
              <div className="flex flex-col gap-0.5 opacity-70 font-bold uppercase tracking-widest" style={{ fontSize: (els.info.fontSize || 12) - 4 }}><span>{student.nis} / {student.nisn || '-'}</span></div>
            </>
          )}
          {showValid && <span className="opacity-70 font-bold uppercase tracking-widest mt-0.5" style={{ fontSize: (els.info.fontSize || 12) - 4 }}>Berlaku: {student.valid_until}</span>}
        </div>
      )}

      {showStamp && settings?.stamp_id && (
        <div className="absolute z-10" style={{ left: els.stamp?.x || 130, top: els.stamp?.y || 380, width: '40px', height: '40px', transform: `scale(${els.stamp?.scale || 0.8})`, transformOrigin: 'top left' }}>
          <Image src={settings.stamp_id} alt="S" fill className="object-contain" unoptimized />
        </div>
      )}
      ...
      {showSig && (
        <>
          ...
          <div className="absolute z-10 text-left" style={{ left: els.principalInfo?.x || 150, top: els.principalInfo?.y || 395, width: '120px' }}>
            <p className="font-bold uppercase opacity-80 leading-none border-t border-slate-300 pt-1" style={{ fontSize: els.principalInfo?.fontSize || 7 }}>{settings?.principal_name}</p>
